...
export function decode(base32Str) {
    return base32.decode.asBytes(addPad(base32Str));
}

// Decodes from Base32 after adding the padding
export function decodeAsString(base32Str) {
    return base32.decode(addPad(base32Str));
}

// Encodes to Base32 and removes the padding
export function encode(bytes) {
    return rmPad(base32.encode(bytes));
}
